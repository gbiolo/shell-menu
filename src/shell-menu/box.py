...
class Box:
    ''' This class rappresent a generic object "Box" that may contains a menu or
    one (or even more) info.
    '''

    def __init__( self ):
        ''' Initialization of an empty box
        The generic attributes are:
            rows  : array containing the rows that compose the box, don't care if
                    it's a menu or an info box, with header and footer
            size  : width of the box expressed in character
            title : title of the box
            index : index used by the method get_row to keep the index of the last
                    row returned to the caller
        '''
        self.rows   = []
        self.size   = 0
        self.title  = ""
    ...
    def format_string( self, unformatted, length, allign="sx" ):
        ...
        Input parameters are the following:
            ...
            length      : final length of the formatted string; in case of input
                          string longer than the final length, the input string won't
                          be change, and the final string will be the same of the
                          input one
            alligm      : allignment of the final string; the allowed values are:
                              - sx     : left allignment
                              - dx     : right allignment
    ...
    def create_header( self ):
        ...
        If the header already exists, the whole content of the "rows" array will be
        erased and replaced by a new header
        '''
        if len( self.rows ) > 0:
            self.rows = []
    ...
    def get_row( self ):
        ...
        If the index is over the array elements number, the method return a string
        of the same size of a row but composed by only spaces (useful to create
        many levels shell-menu)
        '''
