...
class Menu( Box ):
    ...
    def __init__( self, configuration ):
        ''' Initialization of the menu box object, adding to the box the attribute
        "links", that is a dictionary containing all commands indicated into the
        user configuration JSON (secondary one).
        The dictionary uses as keys the indexes of the commands, and as values the
        commands to execute (expressed as string)
        '''
        ...
        index = int( configuration["base"], base=10 )
        for command in commands:
    ...
    def get_command( self, index ):
        ''' Method that return an array containing the command to execute, splitted
        by spaces (the first element is the program, the following are the arguments).
        If the "index" argument value is not a valid key for the links dictionary,
        the method will return the value "None" (no command with the passed index)
        '''
